...
pub fn check_para(text: &str, excluded: &[(usize, usize)]) -> Vec<(usize, usize)> {
    let dict = dict();
    let in_excl = |b: usize| excluded.iter().any(|&(s, e)| b >= s && b < e);
    let mut errors = Vec::new();
    let len = text.len();
    let mut i = 0;
    while i < len {
        if in_excl(i) {
            i = excluded.iter().filter(|&&(s, e)| i >= s && i < e).map(|&(_, e)| e).max().unwrap_or(i + 1);
            continue;
        }
        ...
        if c.is_alphabetic() {
            ...
            if !clean.is_empty() && !dict.contains(clean) { errors.push((byte_start, byte_end)); }
        } else {
            i += c.len_utf8();
        }
    }
